//#region node_modules/.pnpm/@vue+shared@3.6.0-alpha.3/node_modules/@vue/shared/dist/shared.esm-bundler.js
/**
...
//#endregion
//#region node_modules/.pnpm/@vue+reactivity@3.6.0-alpha.3/node_modules/@vue/reactivity/dist/reactivity.esm-bundler.js
			...
			if (flags & 1) {
				...
				if (subSubs !== void 0) {
					...
					if (subSubs.nextSub !== void 0) {
						...
						next$1 = link2.nextSub;
					}
					continue;
				}
			}
		}
		if ((link2 = next$1) !== void 0) {
			next$1 = link2.nextSub;
			continue;
		}
...
function flush() {
	while (notifyIndex < notifyBufferLength) {
		const effect$1 = notifyBuffer[notifyIndex];
...
function checkDirty(link2, sub) {
	...
	top: do {
		...
		if (sub.flags & 16) dirty = true;
			...
			if (link2.nextSub !== void 0 || link2.prevSub !== void 0) stack$1 = {
				value: link2,
				prev: stack$1
			};
			link2 = dep.deps;
...
function setupFlagsHandler(target) {
	...
	Object.defineProperty(target, "flags", {
		...
		set(value) {
			if (!(target._flags & (ReactiveFlags$1.Dirty | ReactiveFlags$1.Pending)) && !!(value & (ReactiveFlags$1.Dirty | ReactiveFlags$1.Pending))) onTrigger(this);
			target._flags = value;
		}
	});
}
var Dep = class {
	constructor(map$1, key) {
		this.map = map$1;
		this.key = key;
		this._subs = void 0;
...
var ReactiveEffect = class {
	...
	resume() {
		if ((this.flags &= -257) & (ReactiveFlags$1.Dirty | ReactiveFlags$1.Pending)) this.notify();
	}
	notify() {
...
function watch$1(source, cb, options = EMPTY_OBJ) {
	const effect$1 = new WatcherEffect(source, cb, options);
	effect$1.run(true);
	const stop$1 = effect$1.stop.bind(effect$1);
	stop$1.pause = effect$1.pause.bind(effect$1);
	stop$1.resume = effect$1.resume.bind(effect$1);
	stop$1.stop = stop$1;
	return stop$1;
}
...
//#endregion
//#region node_modules/.pnpm/@vue+runtime-core@3.6.0-alpha.3/node_modules/@vue/runtime-core/dist/runtime-core.esm-bundler.js
var stack = [];
...
function rerender(id, newRender) {
	...
	[...record.instances].forEach((instance) => {
		...
		isHmrUpdating = true;
		if (instance.vapor) instance.hmrRerender();
		else {
			const i = instance;
...
function reload(id, newComp) {
	...
	if (newComp.__vapor) for (const instance of instances) instance.hmrReload(newComp);
...
var TeleportImpl = {
	...
	process(n1, n2, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, internals) {
		const { mc: mountChildren, pc: patchChildren, pbc: patchBlockChildren, o: { insert: insert$1, querySelector: querySelector$1, createText, createComment: createComment$1 } } = internals;
		...
		if (n1 == null) {
			const placeholder = n2.el = createComment$1("teleport start");
			const mainAnchor = n2.anchor = createComment$1("teleport end");
			insert$1(placeholder, container, anchor);
			insert$1(mainAnchor, container, anchor);
			...
			const mountToTarget = () => {
				const target = n2.target = resolveTarget(n2.props, querySelector$1);
				const targetAnchor = prepareAnchor(target, n2, createText, insert$1);
				if (target) {
			...
			if (disabled) {
				...
				const nextTarget = n2.target = resolveTarget(n2.props, querySelector$1);
	...
	remove(vnode, parentComponent, parentSuspense, { um: unmount, o: { remove: hostRemove } }, doRemove) {
		...
		if (shapeFlag & 16) {
			...
			for (let i = 0; i < children.length; i++) {
				...
				unmount(child$1, parentComponent, parentSuspense, shouldRemove, !!child$1.dynamicChildren);
			}
		}
	},
	move: moveTeleport,
...
function moveTeleport(vnode, container, parentAnchor, { o: { insert: insert$1 }, m: move }, parentComponent, moveType = 2) {
	if (moveType === 0) insert$1(vnode.targetAnchor, container, parentAnchor);
	const { el, anchor, shapeFlag, children, props } = vnode;
	const isReorder = moveType === 2;
	if (isReorder) insert$1(el, container, parentAnchor);
	if (!isReorder || isTeleportDisabled(props)) {
		if (shapeFlag & 16) for (let i = 0; i < children.length; i++) move(children[i], container, parentAnchor, 2, parentComponent);
	}
	if (isReorder) insert$1(anchor, container, parentAnchor);
}
function hydrateTeleport(node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized, { o: { nextSibling, parentNode: parentNode$1, querySelector: querySelector$1, insert: insert$1, createText } }, hydrateChildren) {
	function hydrateDisabledTeleport(node2, vnode2, targetStart, targetAnchor) {
		vnode2.anchor = hydrateChildren(nextSibling(node2), vnode2, parentNode$1(node2), parentComponent, parentSuspense, slotScopeIds, optimized);
		vnode2.targetStart = targetStart;
		vnode2.targetAnchor = targetAnchor;
	}
	const target = vnode.target = resolveTarget(vnode.props, querySelector$1);
	...
	if (target) {
		...
		if (vnode.shapeFlag & 16) if (disabled) hydrateDisabledTeleport(node, vnode, targetNode, targetNode && nextSibling(targetNode));
			...
			if (!vnode.targetAnchor) prepareAnchor(target, vnode, createText, insert$1);
...
function prepareAnchor(target, vnode, createText, insert$1) {
	const targetStart = vnode.targetStart = createText("");
	const targetAnchor = vnode.targetAnchor = createText("");
	targetStart[TeleportEndKey] = targetAnchor;
	if (target) {
		insert$1(targetStart, target);
		insert$1(targetAnchor, target);
	}
	return targetAnchor;
}
...
var BaseTransitionImpl = {
	...
	setup(props, { slots }) {
		...
		return () => {
			...
			const child$1 = findNonCommentChild(children);
			const rawProps = toRaw(props);
			const { mode } = rawProps;
			checkTransitionMode(mode);
			if (state.isLeaving) return emptyPlaceholder(child$1);
			const innerChild = getInnerChild$1(child$1);
			if (!innerChild) return emptyPlaceholder(child$1);
			...
			if (oldInnerChild && oldInnerChild.type !== Comment$1 && !isSameVNodeType(oldInnerChild, innerChild) && recursiveGetSubtree(instance).type !== Comment$1) {
				...
				if (mode === "out-in" && innerChild.type !== Comment$1) {
					...
					return emptyPlaceholder(child$1);
...
function resolveTransitionHooks$1(vnode, props, state, instance, postClone) {
	...
	return baseResolveTransitionHooks({
		...
		cloneHooks: (vnode2) => {
			const hooks = resolveTransitionHooks$1(vnode2, props, state, instance, postClone);
...
function baseResolveTransitionHooks(context, props, state, instance) {
	...
	const hooks = {
		...
		leave(el, remove$2) {
			...
			if (state.isUnmounting) return remove$2();
...
function setTransitionHooks(vnode, hooks) {
	if (vnode.shapeFlag & 6 && vnode.component) if (vnode.type.__vapor) getVaporInterface(vnode.component, vnode).setTransitionHooks(vnode.component, hooks);
	else {
...
function getTransitionRawChildren(children, keepComment = false, parentKey) {
	...
	for (let i = 0; i < children.length; i++) {
		let child$1 = children[i];
		const key = parentKey == null ? child$1.key : String(parentKey) + String(child$1.key != null ? child$1.key : i);
		if (child$1.type === Fragment) {
			if (child$1.patchFlag & 128) keyedFragmentCount++;
			ret = ret.concat(getTransitionRawChildren(child$1.children, keepComment, key));
		} else if (keepComment || child$1.type !== Comment$1) ret.push(key != null ? cloneVNode(child$1, { key }) : child$1);
	}
...
function createHydrationFunctions(rendererInternals) {
	const { mt: mountComponent$1, p: patch, o: { patchProp: patchProp$1, createText, nextSibling, parentNode: parentNode$1, remove: remove$2, insert: insert$1, createComment: createComment$1 } } = rendererInternals;
	const hydrate$1 = (vnode, container) => {
		...
		hydrateNode$1(container.firstChild, vnode, null, null, null);
		flushPostFlushCbs();
		container._vnode = vnode;
	};
	const hydrateNode$1 = (node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized = false) => {
		optimized = optimized || !!vnode.dynamicChildren;
		const isFragmentStart = isComment$1(node) && node.data === "[";
		const onMismatch = () => handleMismatch$1(node, vnode, parentComponent, parentSuspense, slotScopeIds, isFragmentStart);
		...
		switch (type) {
			case Text$1:
				if (domType !== 3) if (vnode.children === "") {
					insert$1(vnode.el = createText(""), parentNode$1(node), node);
					nextNode = node;
  ...
  - expected on client: ${JSON.stringify(vnode.children)}`);
			...
			default: if (shapeFlag & 1) if ((domType !== 1 || vnode.type.toLowerCase() !== node.tagName.toLowerCase()) && !isTemplateNode(node)) nextNode = onMismatch();
				...
				const container = parentNode$1(node);
				if (isFragmentStart) nextNode = locateClosingAnchor(node);
				else if (isComment$1(node) && node.data === "teleport start") nextNode = locateClosingAnchor(node, node.data, "teleport end");
				else nextNode = nextSibling(node);
				if (vnode.type.__vapor) getVaporInterface(parentComponent, vnode).hydrate(vnode, node, container, null, parentComponent);
				else mountComponent$1(vnode, container, null, parentComponent, parentSuspense, getContainerType(container), optimized);
			...
			else if (shapeFlag & 128) nextNode = vnode.type.hydrate(node, vnode, parentComponent, parentSuspense, getContainerType(parentNode$1(node)), slotScopeIds, optimized, rendererInternals, hydrateNode$1);
	...
	const hydrateElement = (el, vnode, parentComponent, parentSuspense, slotScopeIds, optimized) => {
		...
		const forcePatch = type === "input" || type === "option";
		{
			...
			if (shapeFlag & 16 && !(props && (props.innerHTML || props.textContent))) {
				let next$1 = hydrateChildren(el.firstChild, vnode, el, parentComponent, parentSuspense, slotScopeIds, optimized);
			...
			if (node) {
				if (isText && !optimized) {
					if (i + 1 < l && normalizeVNode(children[i + 1]).type === Text$1) {
						insert$1(createText(node.data.slice(vnode.children.length)), container, nextSibling(node));
						node.data = vnode.children;
					}
				}
				node = hydrateNode$1(node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized);
			} else if (isText && !vnode.children) insert$1(vnode.el = createText(""), container);
			else {
...
Server rendered element contains fewer child nodes than client vdom.`);
	...
	const hydrateFragment = (node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized) => {
		...
		const container = parentNode$1(node);
		const next$1 = hydrateChildren(nextSibling(node), vnode, container, parentComponent, parentSuspense, slotScopeIds, optimized);
		if (next$1 && isComment$1(next$1) && next$1.data === "]") return nextSibling(vnode.anchor = next$1);
		else {
			logMismatchError$1();
			insert$1(vnode.anchor = createComment$1(`]`), container, next$1);
			return next$1;
		}
	};
	const handleMismatch$1 = (node, vnode, parentComponent, parentSuspense, slotScopeIds, isFragment$1) => {
...
- expected on client:`, vnode.type);
		...
		if (isFragment$1) {
			...
			while (true) {
				...
				if (next2 && next2 !== end) remove$2(next2);
				else break;
			}
		}
		const next$1 = nextSibling(node);
		const container = parentNode$1(node);
		remove$2(node);
		patch(null, vnode, container, next$1, parentComponent, parentSuspense, getContainerType(container), slotScopeIds);
		if (parentComponent) {
	...
	return [hydrate$1, hydrateNode$1];
}
...
function forEachElement(node, cb) {
	if (isComment$1(node) && node.data === "[") {
		let depth = 1;
		let next$1 = node.nextSibling;
		while (next$1) {
			if (next$1.nodeType === 1) {
				if (cb(next$1) === false) break;
			} else if (isComment$1(next$1)) {
				if (next$1.data === "]") {
					if (--depth === 0) break;
				} else if (next$1.data === "[") depth++;
			}
			next$1 = next$1.nextSibling;
		}
	} else cb(node);
...
function defineAsyncComponent(source) {
	...
	return /* @__PURE__ */ defineComponent({
		...
		__asyncHydrate(el, instance, hydrate$1) {
			performAsyncHydrate(el, instance, hydrate$1, getResolvedComp, load, hydrateStrategy);
		},
		get __asyncResolved() {
...
function performAsyncHydrate(el, instance, hydrate$1, getResolvedComp, load, hydrateStrategy) {
	let patched = false;
...
var KeepAlive = {
	...
	setup(props, { slots }) {
		...
		const { renderer: renderer$1 } = sharedContext;
		const { um: _unmount, o: { createElement: createElement$1 } } = renderer$1;
		const storageContainer = createElement$1("div");
		...
		sharedContext.activate = (vnode, container, anchor, namespace, optimized) => {
			activate(vnode, container, anchor, renderer$1, keepAliveInstance, parentSuspense, namespace, optimized);
		};
		sharedContext.deactivate = (vnode) => {
			deactivate(vnode, storageContainer, renderer$1, keepAliveInstance, parentSuspense);
		};
...
function matches(pattern, name) {
	if (isArray(pattern)) return pattern.some((p$2) => matches(p$2, name));
...
function injectHook(type, hook, target = currentInstance, prepend$1 = false) {
	if (target) {
		...
		if (prepend$1) hooks.unshift(wrappedHook);
...
function ensureValidVNode(vnodes) {
	return vnodes.some((child$1) => {
		if (!isVNode(child$1)) return true;
		if (child$1.type === Comment$1) return false;
		if (child$1.type === Fragment && !ensureValidVNode(child$1.children)) return false;
		return true;
	}) ? vnodes : null;
}
...
function normalizePropsOrEmits(props) {
	return isArray(props) ? props.reduce((normalized, p$2) => (normalized[p$2] = null, normalized), {}) : props;
}
...
var uid$1 = 0;
function createAppAPI(mount, unmount, getPublicInstance$1, render$1) {
 ...
 If you want to mount another app on the same host container, you need to unmount the previous app by calling \`app.unmount()\` first.`);
					...
					return getPublicInstance$1(instance);
...
function provide(key, value) {
	const currentInstance$1 = getCurrentGenericInstance();
	if (!currentInstance$1) warn$1(`provide() can only be used inside setup().`);
	else {
...
function resolvePropValue(options, key, value, instance, resolveDefault$1, isAbsent = false) {
	...
	if (opt != null) {
		...
		if (hasDefault && value === void 0) {
			...
			if (opt.type !== Function && !opt.skipFactory && isFunction(defaultValue)) {
				...
				else value = cachedDefaults[key] = resolveDefault$1(defaultValue, instance, key);
...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const mountElement = (vnode, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized) => {
		...
		if (dirs) invokeDirectiveHook(vnode, null, parentComponent, "created");
		setScopeId$1(el, vnode, vnode.scopeId, slotScopeIds, parentComponent);
		if (props) {
	...
	const setScopeId$1 = (el, vnode, scopeId, slotScopeIds, parentComponent) => {
	...
	const mountChildren = (children, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, start = 0) => {
		for (let i = start; i < children.length; i++) {
			const child$1 = children[i] = optimized ? cloneIfMounted(children[i]) : normalizeVNode(children[i]);
			patch(null, child$1, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized);
		}
	};
			...
			if (patchFlag & 16) patchProps(el, oldProps, newProps, parentComponent, namespace);
				...
				if (patchFlag & 8) {
					...
					for (let i = 0; i < propsToUpdate.length; i++) {
						...
						const next$1 = newProps[key];
						if (next$1 !== prev || key === "value") hostPatchProp(el, key, prev, next$1, namespace, parentComponent);
					}
				}
			}
			if (patchFlag & 1) {
	...
	const processComponent = (n1, n2, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized) => {
		...
		else getVaporInterface(parentComponent, n2).mount(n2, container, anchor, parentComponent);
		else getVaporInterface(parentComponent, n2).update(n1, n2, shouldUpdateComponent(n1, n2, optimized));
		else if (n1 == null) if (n2.shapeFlag & 512) parentComponent.ctx.activate(n2, container, anchor, namespace, optimized);
		else mountComponent$1(n2, container, anchor, parentComponent, parentSuspense, namespace, optimized);
		else updateComponent(n1, n2, optimized);
	};
	const mountComponent$1 = (initialVNode, container, anchor, parentComponent, parentSuspense, namespace, optimized) => {
			...
			if (!instance.isMounted) {
				...
				if (el && hydrateNode$1) {
					const hydrateSubTree = () => {
						...
						hydrateNode$1(el, instance.subTree, instance, parentSuspense, null);
				...
				if (m) queuePostRenderEffect(m, void 0, parentSuspense);
				...
				let { next: next$1, bu, u, parent, vnode } = instance;
				{
					const nonHydratedAsyncRoot = locateNonHydratedAsyncRoot(instance);
					if (nonHydratedAsyncRoot) {
						if (next$1) {
							next$1.el = vnode.el;
							updateComponentPreRender(instance, next$1, optimized);
						}
				...
				let originNext = next$1;
				let vnodeHook;
				pushWarningContext(next$1 || instance.vnode);
				toggleRecurse(instance, false);
				if (next$1) {
					next$1.el = vnode.el;
					updateComponentPreRender(instance, next$1, optimized);
				...
				if (vnodeHook = next$1.props && next$1.props.onVnodeBeforeUpdate) invokeVNodeHook(vnodeHook, parent, next$1, vnode);
				toggleRecurse(instance, true);
				startMeasure(instance, `render`);
				...
				endMeasure(instance, `patch`);
				next$1.el = nextTree.el;
				if (originNext === null) updateHOCHostEl(instance, nextTree.el);
				if (u) queuePostRenderEffect(u, void 0, parentSuspense);
				if (vnodeHook = next$1.props && next$1.props.onVnodeUpdated) queuePostRenderEffect(() => invokeVNodeHook(vnodeHook, parent, next$1, vnode), void 0, parentSuspense);
	...
	const setupRenderEffect = (instance, initialVNode, container, anchor, parentSuspense, namespace, optimized) => {
		const effect$1 = instance.effect = new SetupRenderEffect(instance, initialVNode, container, anchor, parentSuspense, namespace, optimized);
		instance.update = effect$1.run.bind(effect$1);
		toggleRecurse(instance, true);
		effect$1.run();
	};
	...
	const move = (vnode, container, anchor, moveType, parentComponent, parentSuspense = null) => {
		...
		if (shapeFlag & 6) {
			if (type.__vapor) getVaporInterface(parentComponent, vnode).move(vnode, container, anchor);
			else move(vnode.component.subTree, container, anchor, moveType, parentComponent);
			return;
		}
		if (shapeFlag & 128) {
	...
	const unmount = (vnode, parentComponent, parentSuspense, doRemove = false, optimized = false) => {
		...
		if (shapeFlag & 256) {
			if (vnode.type.__vapor) getVaporInterface(parentComponent, vnode).deactivate(vnode, parentComponent.ctx.getStorageContainer());
		...
		if (shapeFlag & 6) if (type.__vapor) {
			getVaporInterface(parentComponent, vnode).unmount(vnode, doRemove);
			return;
		} else unmountComponent$1(vnode.component, parentSuspense, doRemove);
		else {
			if (shapeFlag & 128) {
	...
	const remove$2 = (vnode) => {
		const { type, el, anchor, transition } = vnode;
		if (type === Fragment) {
			if (vnode.patchFlag > 0 && vnode.patchFlag & 2048 && transition && !transition.persisted) vnode.children.forEach((child$1) => {
				if (child$1.type === Comment$1) hostRemove(child$1.el);
				else remove$2(child$1);
			});
	...
	const removeFragment = (cur, end) => {
		let next$1;
		while (cur !== end) {
			next$1 = hostNextSibling(cur);
			hostRemove(cur);
			cur = next$1;
		}
		hostRemove(end);
	};
	const unmountComponent$1 = (instance, parentSuspense, doRemove) => {
		if (instance.type.__hmrId) unregisterHMR(instance);
		const { bum, scope, effect: effect$1, subTree, um, m, a } = instance;
		invalidateMount(m);
		invalidateMount(a);
	...
	const getNextHostNode = (vnode) => {
		if (vnode.shapeFlag & 6) {
			if (vnode.type.__vapor) return hostNextSibling(vnode.anchor);
	...
	let hydrate$1;
	let hydrateNode$1;
	if (createHydrationFns) [hydrate$1, hydrateNode$1] = createHydrationFns(internals);
	const mountApp$1 = (app, container, isHydrate, namespace) => {
		...
		app._context.reload = () => {
			...
			cloned.el = null;
			render$1(cloned, container, namespace);
		};
		if (isHydrate && hydrate$1) hydrate$1(vnode, container);
		else render$1(vnode, container, namespace);
		return vnode.component;
	};
	const unmountApp$1 = (app) => {
		render$1(null, app._container);
	};
	return {
		render: render$1,
		hydrate: hydrate$1,
		hydrateNode: hydrateNode$1,
		internals,
		createApp: createAppAPI(mountApp$1, unmountApp$1, getComponentPublicInstance)
	};
}
function resolveChildrenNamespace({ type, props }, currentNamespace) {
	return currentNamespace === "svg" && type === "foreignObject" || currentNamespace === "mathml" && type === "annotation-xml" && props && props.encoding && props.encoding.includes("html") ? void 0 : currentNamespace;
}
function toggleRecurse({ effect: effect$1, job, vapor }, allowed) {
	if (!vapor) if (allowed) {
		effect$1.flags |= 128;
		job.flags |= 2;
	} else {
...
function performTransitionEnter(el, transition, insert$1, parentSuspense, force = false) {
...
function performTransitionLeave(el, transition, remove$2, isElement = true) {
	const performRemove = () => {
		remove$2();
...
var RenderWatcherEffect = class extends WatcherEffect {
	constructor(instance, source, cb, options, flush$1) {
	...
	notify() {
		if (!(this.flags & 256)) {
			const flush$1 = this.flush;
			const job = this.job;
			if (flush$1 === "post") queuePostRenderEffect(job, void 0, job.i ? job.i.suspense : null);
			else if (flush$1 === "pre") queueJob(job, job.i ? job.i.uid : void 0, true);
			else job();
		}
	}
};
function doWatch(source, cb, options = EMPTY_OBJ) {
	const { immediate, deep, flush: flush$1 = "pre", once } = options;
	if (!cb) {
	...
	const runsImmediately = cb && immediate || !cb && flush$1 !== "post";
	let ssrCleanup;
	...
	const effect$1 = new RenderWatcherEffect(instance, source, cb, baseWatchOptions, flush$1);
	if (cb) effect$1.run(true);
	else if (flush$1 === "post") queuePostRenderEffect(effect$1.job, void 0, instance && instance.suspense);
	else effect$1.run(true);
	const stop$1 = effect$1.stop.bind(effect$1);
	stop$1.pause = effect$1.pause.bind(effect$1);
	stop$1.resume = effect$1.resume.bind(effect$1);
	stop$1.stop = stop$1;
	if (isInSSRComponentSetup) {
		if (ssrCleanup) ssrCleanup.push(stop$1);
		else if (runsImmediately) stop$1();
	}
	return stop$1;
}
...
function baseEmit(instance, props, getter, event, ...rawArgs) {
	...
	const modifiers = isModelListener$1 && getModelModifiers(props, event.slice(7), getter);
	if (modifiers) {
	...
	if (!handler && isModelListener$1) handler = getter(props, handlerName = toHandlerKey(hyphenate(event)));
...
function renderComponentRoot(instance) {
	const { type: Component, vnode, proxy, withProxy, propsOptions: [propsOptions], slots, attrs, emit: emit$3, render: render$1, renderCache, props, data, setupState, ctx, inheritAttrs } = instance;
...
function filterSingleRoot(children, recurse = true) {
	...
	for (let i = 0; i < children.length; i++) {
		const child$1 = children[i];
		if (isVNode(child$1)) {
			if (child$1.type !== Comment$1 || child$1.children === "v-if") if (singleRoot) return;
			else {
				singleRoot = child$1;
...
function mountSuspense(vnode, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, rendererInternals) {
	const { p: patch, o: { createElement: createElement$1 } } = rendererInternals;
	const hiddenContainer = createElement$1("div");
...
function patchSuspense(n1, n2, container, anchor, parentComponent, namespace, slotScopeIds, optimized, { p: patch, um: unmount, o: { createElement: createElement$1 } }) {
	const suspense = n2.suspense = n1.suspense;
	suspense.vnode = n2;
	n2.el = n1.el;
	const newBranch = n2.ssContent;
	const newFallback = n2.ssFallback;
	const { activeBranch, pendingBranch, isInFallback, isHydrating: isHydrating$1 } = suspense;
	if (pendingBranch) {
		...
		if (isSameVNodeType(pendingBranch, newBranch)) {
			...
			suspense.hiddenContainer = createElement$1("div");
			if (isInFallback) {
...
var hasWarned = false;
function createSuspenseBoundary(vnode, parentSuspense, parentComponent, container, hiddenContainer, anchor, namespace, slotScopeIds, optimized, rendererInternals, isHydrating$1 = false) {
	if (!hasWarned) {
		hasWarned = true;
		console[console.info ? "info" : "log"](`<Suspense> is an experimental feature and its API will likely change.`);
	}
	const { p: patch, m: move, um: unmount, n: next$1, o: { parentNode: parentNode$1, remove: remove$2 } } = rendererInternals;
	...
	const suspense = {
		...
		activeBranch: null,
		pendingBranch: null,
		isInFallback: !isHydrating$1,
		isHydrating: isHydrating$1,
		isUnmounted: false,
		effects: [],
			...
			if (suspense.isHydrating) suspense.isHydrating = false;
				...
				if (delayEnter) activeBranch.transition.afterLeave = () => {
					if (pendingId === suspense.pendingId) {
						move(pendingBranch, container2, anchor === initialAnchor ? next$1(activeBranch) : anchor, 0, parentComponent2);
						queuePostFlushCb(effects);
						if (isInFallback && vnode2.ssFallback) vnode2.ssFallback.el = null;
					}
				};
				if (activeBranch) {
					if (parentNode$1(activeBranch.el) === container2) anchor = next$1(activeBranch);
					unmount(activeBranch, parentComponent2, suspense, true);
					if (!delayEnter && isInFallback && vnode2.ssFallback) vnode2.ssFallback.el = null;
				}
				if (!delayEnter) move(pendingBranch, container2, anchor, 0, parentComponent2);
			}
		...
		fallback(fallbackVNode) {
			...
			const anchor2 = next$1(activeBranch);
			const mountFallback = () => {
		...
		next() {
			return suspense.activeBranch && next$1(suspense.activeBranch);
		},
		registerDep(instance, setupRenderEffect, optimized2) {
			...
			instance.asyncDep.catch((err) => {
				...
				setupRenderEffect(instance, vnode2, parentNode$1(hydratedEl || instance.subTree.el), hydratedEl ? null : next$1(instance.subTree), suspense, namespace, optimized2);
				if (placeholder) {
					vnode2.placeholder = null;
					remove$2(placeholder);
				}
...
function hydrateSuspense(node, vnode, parentComponent, parentSuspense, namespace, slotScopeIds, optimized, rendererInternals, hydrateNode$1) {
	...
	const result = hydrateNode$1(node, suspense.pendingBranch = vnode.ssContent, parentComponent, suspense, slotScopeIds, optimized);
...
function normalizeSuspenseSlot(s) {
	...
	if (isArray(s)) {
		...
		if (!singleChild && s.filter((child$1) => child$1 !== NULL_DYNAMIC_COMPONENT).length > 0) warn$1(`<Suspense> slots expect a single root node.`);
		s = singleChild;
	}
	s = normalizeVNode(s);
...
var normalizeRef = ({ ref: ref$1, ref_key, ref_for }, i = currentRenderingInstance) => {
...
function normalizeVNode(child$1) {
	if (child$1 == null || typeof child$1 === "boolean") return createVNode(Comment$1);
	else if (isArray(child$1)) return createVNode(Fragment, null, child$1.slice());
	else if (isVNode(child$1)) return cloneIfMounted(child$1);
	else return createVNode(Text$1, null, String(child$1));
}
function cloneIfMounted(child$1) {
	return child$1.el === null && child$1.patchFlag !== -1 || child$1.memo ? child$1 : cloneVNode(child$1);
}
...
function finishComponentSetup(instance, isSSR, skipOptions) {
	...
	if (!instance.render) {
		if (!isSSR && compile$1 && !Component.render) {
			const template$1 = Component.template || __VUE_OPTIONS_API__ && resolveMergedOptions(instance).template;
			if (template$1) {
				...
				Component.render = compile$1(template$1, finalCompilerOptions);
...
//#endregion
//#region node_modules/.pnpm/@vue+runtime-dom@3.6.0-alpha.3/node_modules/@vue/runtime-dom/dist/runtime-dom.esm-bundler.js
var policy = void 0;
...
var nodeOps = {
	insert: (child$1, parent, anchor) => {
		parent.insertBefore(child$1, anchor || null);
	},
	remove: (child$1) => {
		const parent = child$1.parentNode;
		if (parent) parent.removeChild(child$1);
	},
	...
	insertStaticContent(content, parent, anchor, namespace, start, end) {
		...
		if (start && (start === end || start.nextSibling)) while (true) {
			...
			const template$1 = templateContainer.content;
			if (namespace === "svg" || namespace === "mathml") {
				const wrapper = template$1.firstChild;
				while (wrapper.firstChild) template$1.appendChild(wrapper.firstChild);
				template$1.removeChild(wrapper);
			}
			parent.insertBefore(template$1, anchor);
		}
...
var decorate$1$1 = (t$1) => {
	t$1.displayName = "Transition";
	t$1.props = TransitionPropsValidators;
	return t$1;
};
...
function patchStyle(el, prev, next$1) {
	const style = el.style;
	const isCssString = isString(next$1);
	let hasControlledDisplay = false;
	if (next$1 && !isCssString) {
		if (prev) if (!isString(prev)) {
			for (const key in prev) if (next$1[key] == null) setStyle$1(style, key, "");
		} else for (const prevStyle of prev.split(";")) {
			const key = prevStyle.slice(0, prevStyle.indexOf(":")).trim();
			if (next$1[key] == null) setStyle$1(style, key, "");
		}
		for (const key in next$1) {
			if (key === "display") hasControlledDisplay = true;
			setStyle$1(style, key, next$1[key]);
		}
	} else if (isCssString) {
		if (prev !== next$1) {
			...
			hasControlledDisplay = displayRE.test(next$1);
		}
...
var REMOVAL = {};
/* @__NO_SIDE_EFFECTS__ */
function defineCustomElement(options, extraOptions, _createApp$1) {
	let Comp = defineComponent(options, extraOptions);
	if (isPlainObject(Comp)) Comp = extend({}, Comp, extraOptions);
	class VueCustomElement extends VueElement {
		constructor(initialProps) {
			super(Comp, initialProps, _createApp$1);
		}
	}
...
var VueElement = class VueElement extends BaseClass {
	constructor(_def, _props = {}, _createApp$1 = createApp) {
		super();
		this._def = _def;
		this._props = _props;
		this._createApp = _createApp$1;
		this._isVueCE = true;
		/**
		* @internal
		*/
		...
		*/
		...
		this._ob = null;
		if (this.shadowRoot && _createApp$1 !== createApp) this._root = this.shadowRoot;
		else {
		...
		for (let i = 0; i < outlets.length; i++) {
			...
			if (content) for (const n of content) {
				if (scopeId && n.nodeType === 1) {
					...
					n.setAttribute(id, "");
					let child$1;
					while (child$1 = walker.nextNode()) child$1.setAttribute(id, "");
				}
	...
	setup(props, { slots }) {
		...
		return () => {
			...
			if (children) for (let i = 0; i < children.length; i++) {
				const child$1 = children[i];
				if (child$1.el && child$1.el instanceof Element) {
					prevChildren.push(child$1);
					setTransitionHooks(child$1, resolveTransitionHooks$1(child$1, cssTransitionProps, state, instance));
					positionMap$1.set(child$1, {
						left: child$1.el.offsetLeft,
						top: child$1.el.offsetTop
					});
				}
			}
			...
			for (let i = 0; i < children.length; i++) {
				const child$1 = children[i];
				if (child$1.key != null) setTransitionHooks(child$1, resolveTransitionHooks$1(child$1, cssTransitionProps, state, instance));
				else if (child$1.type !== Text$1) warn(`<TransitionGroup> children must be keyed.`);
			}
...
//#endregion
//#region node_modules/.pnpm/@vue+runtime-vapor@3.6.0-al_ef87b6071e49e32512e979754c1c14bf/node_modules/@vue/runtime-vapor/dist/runtime-vapor.esm-bundler.js
var insertionParent;
var insertionAnchor;
var isLastInsertion;
function setInsertionState(parent, anchor, last) {
	...
	if (anchor !== void 0) if (isHydrating) insertionAnchor = anchor;
	else {
...
function performHydration(fn, setup, cleanup$1) {
	if (!isOptimized$1) {
...
function withHydration(container, fn) {
	...
	const cleanup$1 = () => resetInsertionState();
	return performHydration(fn, setup, cleanup$1);
}
...
function adoptTemplateImpl(node, template$1) {
	if (!(template$1[0] === "<" && template$1[1] === "!")) while (node.nodeType === 8) {
		node = node.nextSibling;
		if (template$1.trim() === "" && isComment(node, "]") && isComment(node.previousSibling, "[")) {
			node.before(node = /* @__PURE__ */ createTextNode());
			break;
		}
	}
	const type = node.nodeType;
	if (type === 8 && !template$1.startsWith("<!") || type === 1 && !template$1.startsWith(`<` + node.tagName.toLowerCase())) node = handleMismatch(node, template$1);
...
function locateHydrationNodeImpl() {
	...
	if (insertionAnchor !== void 0) {
		...
		if (insertionAnchor === 0) node = insertionParent.$lpn = lastPrepend ? locateNextNode(lastPrepend) : firstChild;
		...
		} else node = insertionParent.$lan = lastAppend ? locateNextNode(lastAppend) : insertionAnchor === null ? firstChild : locateChildByLogicalIndex(insertionParent, insertionAnchor);
		insertionParent.$llc = node;
		node.$idx = insertionParent.$curIdx = insertionParent.$curIdx === void 0 ? 0 : insertionParent.$curIdx + 1;
	} else {
		node = currentHydrationNode;
		if (insertionParent && (!node || node.parentNode !== insertionParent)) node = insertionParent.firstChild;
	}
...
function locateEndAnchor(node, open = "[", close = "]") {
	...
	const stack$1 = [node];
	while ((node = node.nextSibling) && stack$1.length > 0) if (node.nodeType === 8) {
		if (node.data === open) stack$1.push(node);
		else if (node.data === close) {
			const matchingOpen = stack$1.pop();
			matchingOpen.$fe = node;
			if (stack$1.length === 0) return node;
		}
	}
	return null;
}
...
function handleMismatch(node, template$1) {
	if (!isMismatchAllowed(node.parentElement, 1)) {
		warn(`Hydration node mismatch:
- rendered on server:`, node, node.nodeType === 3 ? `(text)` : isComment(node, "[[") ? `(start of block node)` : ``, `
- expected on client:`, template$1);
		logMismatchError();
	}
	if (isComment(node, "[")) removeFragmentNodes(node);
	const next$1 = /* @__PURE__ */ _next(node);
	const container = /* @__PURE__ */ parentNode(node);
	remove$1(node, container);
	if (template$1[0] !== "<") return container.insertBefore(/* @__PURE__ */ createTextNode(template$1), next$1);
	const t$1 = /* @__PURE__ */ createElement("template");
	t$1.innerHTML = template$1;
	const newNode = (/* @__PURE__ */ _child(t$1.content)).cloneNode(true);
	newNode.innerHTML = node.innerHTML;
	Array.from(node.attributes).forEach((attr) => {
		newNode.setAttribute(attr.name, attr.value);
	});
	container.insertBefore(newNode, next$1);
	return newNode;
}
...
function removeFragmentNodes(node, endAnchor) {
	...
	while (true) {
		const next$1 = /* @__PURE__ */ _next(node);
		if (next$1 && next$1 !== end) remove$1(next$1, /* @__PURE__ */ parentNode(node));
		else break;
	}
}
/* @__NO_SIDE_EFFECTS__ */
...
var _txt = _child;
var __txt = /* @__NO_SIDE_EFFECTS__ */ (node) => {
...
var txt = /* @__NO_SIDE_EFFECTS__ */ (...args) => {
	return txt.impl(...args);
};
txt.impl = _txt;
var child = /* @__NO_SIDE_EFFECTS__ */ (...args) => {
	return child.impl(...args);
};
child.impl = _child;
var next = /* @__NO_SIDE_EFFECTS__ */ (...args) => {
	return next.impl(...args);
};
next.impl = _next;
var nthChild = /* @__NO_SIDE_EFFECTS__ */ (...args) => {
...
function renderEffect(fn, noLifecycle = false) {
	const effect$1 = new RenderEffect(fn);
	if (noLifecycle) effect$1.fn = fn;
	effect$1.run();
}
var displayName = "VaporTransition";
var decorate$1 = (t$1) => {
	t$1.displayName = displayName;
	t$1.props = TransitionPropsValidators;
	t$1.__vapor = true;
	return t$1;
};
var VaporTransition = /* @__PURE__ */ decorate$1((props, { slots, attrs }) => {
	let resetDisplay;
	if (isHydrating && currentHydrationNode && isTemplateNode(currentHydrationNode)) {
		const { content: { firstChild }, parentNode: parentNode$1 } = currentHydrationNode;
		if (firstChild) {
			...
			parentNode$1.replaceChild(firstChild, currentHydrationNode);
	...
	renderEffect(() => {
		...
		if (isMounted) if (isFragment(children)) children.$transition.props = resolvedProps;
		else {
			const child$1 = findTransitionBlock(children);
			if (child$1) {
				child$1.$transition.props = resolvedProps;
				applyTransitionHooks(child$1, child$1.$transition, void 0, true);
			}
		}
		else isMounted = true;
	});
	let fallthroughAttrs = true;
	if (instance.hasFallthrough) renderEffect(() => {
		const resolvedAttrs = extend({}, attrs);
		const child$1 = findTransitionBlock(children);
		if (child$1) {
			child$1.$root = true;
			applyFallthroughProps(child$1, resolvedAttrs);
	...
	if (resetDisplay && resolvedProps.appear) {
		const child$1 = findTransitionBlock(children);
		hooks.beforeEnter(child$1);
		resetDisplay();
		queuePostFlushCb(() => hooks.enter(child$1));
	}
	return children;
});
...
function resolveTransitionHooks(block, props, state, instance, postClone) {
	const context = getTransitionHooksContext(String(block.$key), props, state, instance, postClone);
...
function applyTransitionHooks(block, hooks, fallthroughAttrs = true, isResolved = false) {
	...
	const child$1 = isResolved ? block : findTransitionBlock(block, isFrag);
	if (!child$1) {
		if (isFrag) setTransitionHooksOnFragment(block, hooks);
		return hooks;
	}
	const { props, instance, state, delayedLeave } = hooks;
	let resolvedHooks = resolveTransitionHooks(child$1, props, state, instance, (hooks2) => resolvedHooks = hooks2);
	resolvedHooks.delayedLeave = delayedLeave;
	child$1.$transition = resolvedHooks;
	if (isFrag) setTransitionHooksOnFragment(block, resolvedHooks);
	if (fallthroughAttrs && instance.hasFallthrough) {
		child$1.$root = true;
		applyFallthroughProps(child$1, instance.attrs);
	}
...
function findTransitionBlock(block, inFragment = false) {
	...
	if (block instanceof Node) {
		if (block instanceof Element) child$1 = block;
	} else if (isVaporComponent(block)) if (isAsyncWrapper(block) && !block.type.__asyncResolved) child$1 = block;
	else {
		if (getComponentName(block.type) === displayName) return void 0;
		child$1 = findTransitionBlock(block.block, inFragment);
		if (child$1 && child$1.$key === void 0) child$1.$key = block.uid;
	}
		...
		inFragment = true;
		if (block.insert) child$1 = block;
		else child$1 = findTransitionBlock(block.nodes, true);
	}
	if (!child$1 && !inFragment) warn("Transition component has no valid child element");
	return child$1;
}
function setTransitionHooksOnFragment(block, hooks) {
	if (isFragment(block)) block.$transition = hooks;
...
var DynamicFragment = class extends VaporFragment {
	constructor(anchorLabel) {
		...
		this.hydrate = (isEmpty = false) => {
			...
			const { parentNode: parentNode$1, nextNode } = findBlockNode(this.nodes);
			queuePostFlushCb(() => {
				parentNode$1.insertBefore(this.anchor = /* @__PURE__ */ createComment(this.anchorLabel), nextNode);
			});
		};
		if (isHydrating) {
			this.anchorLabel = anchorLabel;
			locateHydrationNode();
		} else this.anchor = anchorLabel ? /* @__PURE__ */ createComment(anchorLabel) : /* @__PURE__ */ createTextNode();
	}
	update(render$1, key = render$1) {
		if (key === this.current) {
		...
		if (this.scope) {
			...
			if (mode) {
				applyTransitionLeaveHooks(this.nodes, transition, () => this.render(render$1, instance, transition, parent));
				parent && remove$1(this.nodes, parent);
				if (mode === "out-in") {
					setActiveSub(prevSub);
					return;
				}
			} else parent && remove$1(this.nodes, parent);
		}
		this.render(render$1, instance, transition, parent);
		if (this.fallback) {
			const hasNestedFragment = isFragment(this.nodes);
			if (hasNestedFragment) setFragmentFallback(this.nodes, this.fallback);
			const invalidFragment = findInvalidFragment(this);
			if (invalidFragment) {
				parent && remove$1(this.nodes, parent);
				(this.scope || (this.scope = new EffectScope())).run(() => {
					if (hasNestedFragment) renderFragmentFallback(invalidFragment);
	...
	render(render$1, instance, transition, parent) {
		if (render$1) {
			...
			this.nodes = this.scope.run(render$1) || [];
			if (this.inKeepAlive) instance.cacheFragment(this);
...
function setFragmentFallback(fragment, fallback) {
	...
	if (isFragment(fragment.nodes)) setFragmentFallback(fragment.nodes, fragment.fallback);
}
	...
	ownKeys(target) {
		...
		if (dynamicSources) {
			...
			for (const source of dynamicSources) if (isFunction(source)) {
				const slot = source();
				if (isArray(slot)) for (const s of slot) keys.push(String(s.name));
...
function createSlot(name, rawProps, fallback, noSlotted) {
	...
	if (isRef(rawSlots._)) {
		...
		if (isDynamicName || rawSlots.$) renderEffect(renderSlot$1);
		else renderSlot$1();
	}
	if (!isHydrating) {
		if (!noSlotted) {
...
function setDOMProp(el, key, value, forceHydrate = false) {
	if (!isApplyingFallthroughProps && el.$root && hasFallthroughKey(key)) return;
	if (isHydrating && !attributeHasMismatch(el, key, value) && !shouldForceHydrate(el, key) && !forceHydrate) return;
	const prev = el[key];
...
function setClass(el, value) {
	if (el.$root) setClassIncremental(el, value);
		...
		if (value !== el.$cls) el.className = el.$cls = value;
	}
}
...
function setValue(el, value, forceHydrate = false) {
	...
	el._value = value;
	if (isHydrating && !attributeHasMismatch(el, "value", getClientText(el, value)) && !shouldForceHydrate(el, "value") && !forceHydrate) return;
...
function setDynamicProps(el, args) {
	...
	if (prevKeys) {
		for (const key of prevKeys) if (!(key in props)) setDynamicProp(el, key, null);
	}
	for (const key of el[cacheKey] = Object.keys(props)) setDynamicProp(el, key, props[key]);
}
function setDynamicProp(el, key, value) {
	const isSVG = false;
	let forceHydrate = false;
	if (key === "class") setClass(el, value);
...
var VaporKeepAliveImpl = /* @__PURE__ */ defineVaporComponent({
	name: "VaporKeepAlive",
	__isKeepAlive: true,
	props: {
		include: [
	...
	setup(props, { slots }) {
		...
		function shouldCache(instance) {
			if (isAsyncWrapper(instance) && !instance.type.__asyncResolved) return false;
		...
		onBeforeUnmount(() => {
			cache.forEach((cached, key) => {
				...
				remove$1(cached, storageContainer);
			});
		});
		...
		keepAliveInstance.processFragment = (frag) => {
			...
			if (fragment) {
				if (cache.has(fragment.vnode.type)) fragment.vnode.shapeFlag |= 512;
				if (shouldCache(innerBlock)) fragment.vnode.shapeFlag |= 256;
			} else {
				if (cache.has(innerBlock.type)) innerBlock.shapeFlag |= 512;
				if (shouldCache(innerBlock)) innerBlock.shapeFlag |= 256;
			}
		};
		...
		keepAliveInstance.activate = (instance, parentNode$1, anchor) => {
			current = instance;
			activate$1(instance, parentNode$1, anchor);
		};
		...
		if (isVaporComponent(children)) children.shapeFlag |= 256;
		else if (isInteropFragment(children)) children.vnode.shapeFlag |= 256;
		function pruneCache(filter) {
			cache.forEach((cached, key) => {
				const instance = getInstanceFromCache(cached);
				if (!instance) return;
		...
		function pruneCacheEntry(key) {
			const cached = cache.get(key);
			resetCachedShapeFlag(cached);
			if (cached !== current) remove$1(cached);
			cache.delete(key);
			keys.delete(key);
		}
		watch(() => [props.include, props.exclude], ([include, exclude]) => {
...
function getInnerBlock(block) {
	...
	else if (isInteropFragment(block)) return block.vnode;
...
function findInteropFragment(block) {
	if (isInteropFragment(block)) return block;
	if (isFragment(block)) return findInteropFragment(block.nodes);
}
function getInstanceFromCache(cached) {
	if (isVaporComponent(cached)) return cached;
	return cached.vnode.component;
}
function activate$1(instance, parentNode$1, anchor) {
	insert(instance.block, parentNode$1, anchor);
	queuePostFlushCb(() => {
		instance.isDeactivated = false;
		if (instance.a) invokeArrayFns(instance.a);
	});
...
function findParentKeepAlive(instance) {
	let parent = instance;
	while (parent) {
		if (isKeepAlive(parent)) return parent;
		parent = parent.parent;
	}
	return null;
}
...
var vaporInteropImpl = {
	mount(vnode, container, anchor, parentComponent) {
		let selfAnchor = vnode.el = vnode.anchor = /* @__PURE__ */ createTextNode();
		if (isHydrating) queuePostFlushCb(() => container.insertBefore(selfAnchor, anchor));
		else container.insertBefore(selfAnchor, anchor);
	...
	unmount(vnode, doRemove) {
		...
		if (vnode.component) unmountComponent(vnode.component, container);
		else if (vnode.vb) remove$1(vnode.vb, container);
		remove$1(vnode.anchor, container);
	},
	...
	hydrate(vnode, node, container, anchor, parentComponent) {
		hydrateNode(node, () => this.mount(vnode, container, anchor, parentComponent));
	...
	hydrateSlot(vnode, node) {
		...
		return /* @__PURE__ */ _next(vnode.anchor);
	},
...
var vaporSlotsProxyHandler = { get(target, key) {
	...
	if (isFunction(slot)) slot.__vapor = true;
	return slot;
} };
var vdomHydrateNode;
function createVDOMComponent(internals, component, rawProps, rawSlots) {
	...
	const wrapper = new VaporComponentInstance({ props: component.props }, rawProps, rawSlots);
	vnode.vi = (instance) => {
	...
	let rawRef = null;
	let isMounted = false;
	const unmount = (parentNode$1, transition) => {
		if (rawRef) setRef(rawRef, null, null, vnode, true);
		if (transition) setTransitionHooks(vnode, transition);
		if (vnode.shapeFlag & 256) {
			deactivate(vnode, findParentKeepAlive(parentInstance).getStorageContainer(), internals, parentInstance, null);
			return;
		}
		internals.umt(vnode.component, null, !!parentNode$1);
	};
	frag.hydrate = () => {
	...
	frag.insert = (parentNode$1, anchor, transition) => {
		if (isHydrating) return;
		if (vnode.shapeFlag & 512) activate(vnode, parentNode$1, anchor, internals, parentInstance, null, void 0, false);
		else {
			const prev = currentInstance;
			simpleSetCurrentInstance(parentInstance);
			if (!isMounted) {
				if (transition) setTransitionHooks(vnode, transition);
				internals.mt(vnode, parentNode$1, anchor, parentInstance, null, void 0, false);
				if (rawRef) setRef(rawRef, null, null, vnode);
				onScopeDispose(unmount, true);
				isMounted = true;
			} else internals.m(vnode, parentNode$1, anchor, 2, parentInstance);
			simpleSetCurrentInstance(prev);
		}
		frag.nodes = vnode.el;
	};
	frag.remove = unmount;
	frag.setRef = (instance, ref$1, refFor, refKey) => {
		rawRef = normalizeRef({
			ref: ref$1,
			ref_for: refFor,
			ref_key: refKey
...
function renderVDOMSlot(internals, slotsRef, name, props, parentComponent, fallback) {
	...
	frag.insert = (parentNode$1, anchor) => {
		if (isHydrating) return;
		if (!isMounted) {
			render$1(parentNode$1, anchor);
			isMounted = true;
		} else internals.m(oldVNode, parentNode$1, anchor, 2, parentComponent);
		frag.remove = (parentNode2) => {
			if (fallbackNodes) remove$1(fallbackNodes, parentNode2);
			else if (oldVNode) internals.um(oldVNode, parentComponent, null);
		};
	};
	const render$1 = (parentNode$1, anchor) => {
		renderEffect(() => {
			let vnode;
			...
			if (isValidSlot) if (isHydrating) {
				...
				if (fallbackNodes) {
					remove$1(fallbackNodes, parentNode$1);
					fallbackNodes = void 0;
				}
				internals.p(oldVNode, vnode, parentNode$1, anchor, parentComponent, null, void 0, vnode.slotScopeIds);
				oldVNode = vnode;
				frag.nodes = vnode.el;
			}
			else {
				...
				if (fallback && !fallbackNodes) {
					...
					if (isHydrating) {
						...
						insert(fallbackNodes, parentNode$1, anchor);
...
function hydrateVNode(vnode, parentComponent) {
	locateHydrationNode();
	let node = currentHydrationNode;
	while (isComment(node, "[") && vnode.type !== Fragment && !(isVaporComponent(parentComponent) && isRef(parentComponent.rawSlots._))) node = node.nextSibling;
	if (currentHydrationNode !== node) setCurrentHydrationNode(node);
	if (!vdomHydrateNode) vdomHydrateNode = ensureHydrationRenderer().hydrateNode;
	const nextNode = vdomHydrateNode(currentHydrationNode, vnode, parentComponent, null, null, false);
	...
	if (isArray(fallbackNodes) && fallbackNodes.every(isVNode)) {
		...
		frag.insert = (parentNode$1, anchor) => {
			fallbackNodes.forEach((vnode) => {
				internals.p(null, vnode, parentNode$1, anchor, parentComponent);
			});
		};
		frag.remove = (parentNode$1) => {
			fallbackNodes.forEach((vnode) => {
	...
	const getProp = (instance, key) => {
		...
		if (dynamicSources) {
			...
			while (i--) {
				...
				for (rawKey in source) if (camelize(rawKey) === key) return resolvePropValue(propsOptions, key, isDynamic ? source[rawKey] : source[rawKey](), instance, resolveDefault);
			}
		}
		for (const rawKey in rawProps) if (camelize(rawKey) === key) return resolvePropValue(propsOptions, key, rawProps[rawKey](), instance, resolveDefault);
...
function getAttrFromRawProps(rawProps, key) {
	...
	if (hasOwn(rawProps, key)) if (merged) merged.push(rawProps[key]());
	else return rawProps[key]();
...
function resolveDynamicProps(props) {
	...
	for (const key in props) if (key !== "$") mergedRawProps[key] = props[key]();
...
var TeleportFragment = class extends VaporFragment {
	constructor(props, slots) {
		...
		this.remove = (parent = this.parent) => {
			if (this.nodes) {
				remove$1(this.nodes, this.mountContainer);
				this.nodes = [];
			}
			if (this.targetStart) {
				remove$1(this.targetStart, this.target);
				this.targetStart = void 0;
				remove$1(this.targetAnchor, this.target);
				this.targetAnchor = void 0;
			}
			if (this.anchor) {
				remove$1(this.anchor, this.anchor.parentNode);
				this.anchor = void 0;
			}
			if (this.placeholder) {
				remove$1(this.placeholder, parent);
		...
		this.hydrate = () => {
			...
			if (target) {
				...
				if (disabled) this.hydrateDisabledTeleport(targetNode);
					...
					if (!this.targetAnchor) this.mount(target);
		...
		this.rawProps = props;
		this.rawSlots = slots;
		this.anchor = isHydrating ? void 0 : /* @__PURE__ */ createComment("teleport end");
		renderEffect(() => {
			this.resolvedProps = extend({}, new Proxy(this.rawProps, rawPropsProxyHandlers));
			this.handlePropsUpdate();
		});
		if (!isHydrating) this.initChildren();
	}
	get parent() {
	...
	handlePropsUpdate() {
		...
		if (isTeleportDisabled(this.resolvedProps)) mount(this.parent, this.anchor);
		else if (isTeleportDeferred(this.resolvedProps)) queuePostFlushCb(mountToTarget);
		else mountToTarget();
	}
...
function insert(block, parent, anchor = null, parentSuspense) {
	...
	if (block instanceof Node) {
		if (!isHydrating) if (block instanceof Element && block.$transition && !block.$transition.disabled) performTransitionEnter(block, block.$transition, () => parent.insertBefore(block, anchor), parentSuspense);
...
function remove$1(block, parent) {
	...
	else if (isArray(block)) for (let i = 0; i < block.length; i++) remove$1(block[i], parent);
	else {
		if (block.remove) block.remove(parent, block.$transition);
		else remove$1(block.nodes, parent);
		if (block.anchor) remove$1(block.anchor, parent);
		if (block.scope) {
			block.scope.stop();
...
function normalizeBlock(block) {
	...
	else if (isArray(block)) block.forEach((child$1) => nodes.push(...normalizeBlock(child$1)));
	else if (isVaporComponent(block)) nodes.push(...normalizeBlock(block.block));
	else if (block instanceof TeleportFragment) nodes.push(block.placeholder, block.anchor);
	else {
...
function findBlockNode(block) {
	let { parentNode: parentNode$1, nextSibling: nextNode } = findLastChild(block);
...
function hmrRerender(instance) {
	...
	remove$1(instance.block, parent);
	const prev = setCurrentInstance(instance);
...
function createComponent(component, rawProps, rawSlots, isSingleRoot, once, appContext = currentInstance && currentInstance.appContext || emptyContext) {
	...
	if (isSingleRoot && component.inheritAttrs !== false && isVaporComponent(currentInstance) && currentInstance.hasFallthrough) {
		...
		if (rawProps) (rawProps.$ || (rawProps.$ = [])).push(() => attrs);
		else rawProps = { $: [() => attrs] };
	}
	if (currentInstance && currentInstance.vapor && isKeepAlive(currentInstance)) {
		const cached = currentInstance.getCachedComponent(component);
		if (cached) return cached;
	}
	if (appContext.vapor && !component.__vapor) {
		const frag = appContext.vapor.vdomMount(component, rawProps, rawSlots);
		if (!isHydrating) {
	...
	if (isHydrating && isAsyncWrapper(instance) && component.__asyncHydrate && !component.__asyncResolved) {
		...
		component.__asyncHydrate(el, instance, () => setupComponent(instance, component));
	} else setupComponent(instance, component);
	onScopeDispose(() => unmountComponent(instance), true);
	if (_insertionParent || isHydrating) mountComponent(instance, _insertionParent, _insertionAnchor);
	if (isHydrating && _insertionAnchor !== void 0) advanceHydrationNode(_insertionParent);
	return instance;
}
function setupComponent(instance, component) {
	...
	else if (!setupFn && component.render) instance.block = callWithErrorHandling(component.render, instance, 1);
	else instance.block = setupResult;
	if (instance.hasFallthrough && component.inheritAttrs !== false && Object.keys(instance.attrs).length) renderEffect(() => applyFallthroughProps(instance.block, instance.attrs));
	setActiveSub(prevSub);
	setCurrentInstance(...prevInstance);
	popWarningContext();
	endMeasure(instance, "init");
...
function createComponentWithFallback(comp, rawProps, rawSlots, isSingleRoot, once, appContext) {
	...
	const el = isHydrating ? adoptTemplate(currentHydrationNode, `<${comp}/>`) : /* @__PURE__ */ createElement(comp);
	...
	if (rawSlots) {
		...
		if (rawSlots.$);
		else insert(getSlot(rawSlots, "default")(), el);
		if (isHydrating) setCurrentHydrationNode(nextNode);
	}
	if (!isHydrating) {
...
function unmountComponent(instance, parentNode$1) {
	if (parentNode$1 && instance.parent && instance.parent.vapor && instance.shapeFlag & 256) {
	...
	if (parentNode$1) remove$1(instance.block, parentNode$1);
}
...
var mountApp = (app, container) => {
	...
	const instance = createComponent(app._component, app._props, null, false, false, app._context);
...
var hydrateApp = (app, container) => {
	...
	withHydration(container, () => {
		instance = createComponent(app._component, app._props, null, false, false, app._context);
		mountComponent(instance, container);
		flushOnAppMount();
	});
	return instance;
	...
	return /* @__PURE__ */ defineVaporComponent({
		...
		setup() {
			...
			renderEffect(() => {
				resolvedComp = getResolvedComp();
				let render$1;
				if (loaded.value && resolvedComp) render$1 = () => createInnerComp(resolvedComp, instance, frag);
				else if (error.value && errorComponent) render$1 = () => createComponent(errorComponent, { error: () => error.value });
				else if (loadingComponent && !delayed.value) render$1 = () => createComponent(loadingComponent);
				if (instance.$transition) frag.$transition = instance.$transition;
				frag.update(render$1);
			});
			return frag;
		}
	});
...
function createInnerComp(comp, parent, frag) {
	...
	const instance = createComponent(comp, rawProps, rawSlots, isSingleRoot, void 0, appContext);
...
function template(html, root) {
	...
	return () => {
		...
		if (!node) {
			t = t || /* @__PURE__ */ createElement("template");
			t.innerHTML = html;
...
function createIf(condition, b1, b2, once) {
	...
	if (!isHydrating) resetInsertionState();
	let frag;
	if (once) frag = condition() ? b1() : b2 ? b2() : [];
	else {
...
function createKeyedFragment(key, render$1) {
	const _insertionParent = insertionParent;
	const _insertionAnchor = insertionAnchor;
	const _isLastInsertion = isLastInsertion;
	if (!isHydrating) resetInsertionState();
...
var createFor = (src, renderItem, getKey, flags = 0, setup) => {
	...
	let currentKey;
	let parentAnchor;
	if (!isHydrating) parentAnchor = /* @__PURE__ */ createComment("for");
	...
	const renderList$1 = () => {
		...
		if (!isMounted) {
			...
			for (let i = 0; i < newLength; i++) {
				...
				if (isHydrating) setCurrentHydrationNode(findBlockNode(nodes).nextNode);
			}
			if (isHydrating) {
				...
				if (_insertionParent) updateLastLogicalChild(_insertionParent, parentAnchor);
			}
		} else {
			parent = parent || parentAnchor.parentNode;
			if (!oldLength) {
				if (frag.fallback && frag.nodes[0].length > 0) remove$1(frag.nodes[0], parent);
	...
	const mount = (source, idx, anchor = parentAnchor, [item, key, index] = getItem(source, idx), key2 = getKey && getKey(item, key, index)) => {
		...
		if (frag.$transition) applyTransitionHooks(block.nodes, frag.$transition, false);
...
function createForSlots(rawSource, getSlot$1) {
	const source = normalizeSource(rawSource);
	const sourceLength = source.values.length;
	const slots = new Array(sourceLength);
	for (let i = 0; i < sourceLength; i++) slots[i] = getSlot$1(...getItem(source, i));
	return slots;
}
...
function createTemplateRefSetter() {
	const instance = currentInstance;
	return (...args) => setRef$1(instance, ...args);
}
function setRef$1(instance, el, ref$1, oldRef, refFor = false, refKey) {
	if (!instance || instance.isUnmounted) return;
	if (isFragment(el) && el.setRef) {
		el.setRef(instance, ref$1, refFor, refKey);
		return;
	}
	if (isVaporComponent(el) && isAsyncWrapper(el)) {
		...
		if (!i.type.__asyncResolved) {
			frag.setRef = (i2) => setRef$1(instance, i2, ref$1, oldRef, refFor);
			return;
		}
		el = frag.nodes;
	}
	...
	if (isFunction(ref$1)) {
		const invokeRefSetter = (value) => {
			callWithErrorHandling(ref$1, currentInstance, 12, [value, refs]);
		};
		...
		if (_isString || _isRef) {
			const doSet = () => {
				if (refFor) {
					existing = _isString ? canSetSetupRef(ref$1) ? setupState[ref$1] : refs[ref$1] : ref$1.value;
					if (!isArray(existing)) {
						existing = [refValue];
						if (_isString) {
							refs[ref$1] = existing;
							if (canSetSetupRef(ref$1)) {
								setupState[ref$1] = refs[ref$1];
								existing = setupState[ref$1];
							}
						} else {
							ref$1.value = existing;
							if (refKey) refs[refKey] = existing;
						}
					} else if (!existing.includes(refValue)) existing.push(refValue);
				} else if (_isString) {
					refs[ref$1] = refValue;
					if (canSetSetupRef(ref$1)) setupState[ref$1] = refValue;
				} else if (_isRef) {
					ref$1.value = refValue;
					if (refKey) refs[refKey] = refValue;
				} else warn("Invalid template ref type:", ref$1, `(${typeof ref$1})`);
			};
			...
			onScopeDispose(() => {
				queuePostFlushCb(() => {
					if (isArray(existing)) remove(existing, refValue);
						...
						refs[ref$1] = null;
						if (canSetSetupRef(ref$1)) setupState[ref$1] = null;
					} else if (_isRef) {
						ref$1.value = null;
...
function createDynamicComponent(getter, rawProps, rawSlots, isSingleRoot, once) {
	...
	const renderFn = () => {
		...
		frag.update(() => createComponentWithFallback(resolveDynamicComponent(value), rawProps, rawSlots, isSingleRoot, once, appContext), value);
	};
	if (once) renderFn();
...
function applyVShow(target, source) {
	...
	if (target instanceof DynamicFragment) {
		...
		target.update = (render$1, key) => {
			update.call(target, render$1, key);
			setDisplay(target, source());
		};
	} else if (target instanceof VaporFragment && target.insert) {
		const insert$1 = target.insert;
		target.insert = (parent, anchor) => {
			insert$1.call(target, parent, anchor);
			setDisplay(target, source());
		};
	}
...
function setDisplay(target, value) {
	if (isVaporComponent(target)) return setDisplay(target, value);
	if (isArray(target)) {
		if (target.length === 0) return;
		if (target.length === 1) return setDisplay(target[0], value);
	}
	if (target instanceof DynamicFragment) return setDisplay(target.nodes, value);
	if (target instanceof VaporFragment && target.insert) return setDisplay(target.nodes, value);
...
var applyDynamicModel = (el, get, set, modifiers) => {
	let apply$1 = applyTextModel;
	if (el.tagName === "SELECT") apply$1 = applySelectModel;
	else if (el.tagName === "TEXTAREA") apply$1 = applyTextModel;
	else if (el.type === "checkbox") apply$1 = applyCheckboxModel;
	else if (el.type === "radio") apply$1 = applyRadioModel;
	apply$1(el, get, set, modifiers);
};
...
var VaporTransitionGroup = decorate({
	...
	setup(props, { slots }) {
		...
		onBeforeUpdate(() => {
			...
			if (children) for (let i = 0; i < children.length; i++) {
				const child$1 = children[i];
				if (isValidTransitionBlock(child$1)) {
					prevChildren.push(child$1);
					child$1.$transition.disabled = true;
					positionMap.set(child$1, getTransitionElement(child$1).getBoundingClientRect());
				}
			}
		});
		onUpdated(() => {
			...
			prevChildren.forEach((child$1) => {
				child$1.$transition.disabled = false;
				recordPosition(child$1);
			});
		...
		for (let i = 0; i < children.length; i++) {
			const child$1 = children[i];
			if (isValidTransitionBlock(child$1)) {
				if (child$1.$key != null) setTransitionHooks$1(child$1, resolveTransitionHooks(child$1, cssTransitionProps, state, instance));
				else if (child$1.$key == null) warn(`<transition-group> children must be keyed`);
			}
		}
		const tag = props.tag;
		if (tag) {
...
function getFirstConnectedChild(children) {
	for (let i = 0; i < children.length; i++) {
		const child$1 = children[i];
		const el = getTransitionElement(child$1);
		if (el.isConnected) return el;
	}
}

//#endregion
//#region node_modules/.pnpm/vue@3.6.0-alpha.3_typescript@5.9.3/node_modules/vue/dist/vue.runtime.esm-bundler.js
function initDev() {
	initCustomFormatter();
}
initDev();
...
//#endregion
export { BaseTransition, BaseTransitionPropsValidators, Comment$1 as Comment, DeprecationTypes, EffectScope, ErrorCodes, ErrorTypeStrings, Fragment, KeepAlive, MismatchTypes, MoveType, ReactiveEffect, Static, Suspense, Teleport, Text$1 as Text, TrackOpTypes, Transition, TransitionGroup, TransitionPropsValidators, TriggerOpTypes, VaporFragment, VaporKeepAliveImpl, VaporTeleportImpl, VaporTransition, VaporTransitionGroup, VueElement, activate, addTransitionClass, applyCheckboxModel, applyDynamicModel, applyRadioModel, applySelectModel, applyTextModel, applyVShow, assertNumber, baseApplyTranslation, baseEmit, baseNormalizePropsOptions, baseResolveTransitionHooks, callPendingCbs, callWithAsyncErrorHandling, callWithErrorHandling, camelize, capitalize, checkTransitionMode, child, cloneVNode, compatUtils, compile, computed$1 as computed, createApp, createAppAPI, createAsyncComponentContext, createBaseVNode, createBlock, createCanSetSetupRefChecker, createCommentVNode, createComponent, createComponentWithFallback, createDynamicComponent, createElementBlock, createFor, createForSlots, createHydrationRenderer, createIf, createInternalObject, createKeyedFragment, createPropsRestProxy, createRenderer, createSSRApp, createSlot, createSlots, createStaticVNode, createTemplateRefSetter, createTextNode, createTextVNode, createVNode, createVaporApp, createVaporSSRApp, currentInstance, customRef, deactivate, defineAsyncComponent, defineComponent, defineCustomElement, defineEmits, defineExpose, defineModel, defineOptions, defineProps, defineSSRCustomElement, defineSlots, defineVaporAsyncComponent, defineVaporComponent, delegate, delegateEvents, devtools, devtoolsComponentAdded, effect, effectScope, endMeasure, ensureHydrationRenderer, ensureRenderer, ensureVaporSlotFallback, expose, flushOnAppMount, forceReflow, getAttributeMismatch, getComponentName, getCurrentInstance, getCurrentScope, getCurrentWatcher, getDefaultValue, getInheritedScopeIds, getRestElement, getTransitionRawChildren, getValue, guardReactiveProps, h, handleError, handleMovedChildren, hasCSSTransform, hasInjectionContext, hydrate, hydrateOnIdle, hydrateOnInteraction, hydrateOnMediaQuery, hydrateOnVisible, initCustomFormatter, initDirectivesForSSR, initFeatureFlags, inject, insert, isAsyncWrapper, isEmitListener, isFragment, isKeepAlive, isMapEqual, isMemoSame, isMismatchAllowed, isProxy, isReactive, isReadonly, isRef, isRuntimeOnly, isSetEqual, isShallow, isTeleportDeferred, isTeleportDisabled, isTemplateNode, isVNode, isValidHtmlOrSvgAttribute, isVaporComponent, leaveCbKey, markAsyncBoundary, markRaw, matches, mergeDefaults, mergeModels, mergeProps, moveCbKey, next, nextTick, nextUid, normalizeClass, normalizeContainer, normalizeProps, normalizeRef, normalizeStyle, nthChild, on, onActivated, onBeforeMount, onBeforeUnmount, onBeforeUpdate, onDeactivated, onErrorCaptured, onMounted, onRenderTracked, onRenderTriggered, onScopeDispose, onServerPrefetch, onUnmounted, onUpdated, onWatcherCleanup, openBlock, patchStyle, performAsyncHydrate, performTransitionEnter, performTransitionLeave, popScopeId, popWarningContext, prepend, provide, proxyRefs, pushScopeId, pushWarningContext, queueJob, queuePostFlushCb, reactive, readonly, ref, registerHMR, registerRuntimeCompiler, remove$1 as remove, removeTransitionClass, render, renderEffect, renderList, renderSlot, resetShapeFlag, resolveComponent, resolveDirective, resolveDynamicComponent, resolveFilter, resolvePropValue, resolveTarget, resolveTransitionHooks$1 as resolveTransitionHooks, resolveTransitionProps, setAttr, setBlockHtml, setBlockText, setBlockTracking, setClass, setCurrentInstance, setDOMProp, setDevtoolsHook, setDynamicEvents, setDynamicProps, setElementText, setHtml, setInsertionState, setProp, setRef, setStyle, setText, setTransitionHooks, setValue, shallowReactive, shallowReadonly, shallowRef, shouldSetAsProp, simpleSetCurrentInstance, ssrContextKey, ssrUtils, startMeasure, stop, template, toClassSet, toDisplayString, toHandlerKey, toHandlers, toRaw, toRef, toRefs, toStyleMap, toValue, transformVNodeArgs, triggerRef, txt, unref, unregisterHMR, unsafeToTrustedHTML, useAsyncComponentState, useAttrs, useCssModule, useCssVars, useHost, useId, useModel, useSSRContext, useShadowRoot, useSlots, useTemplateRef, useTransitionState, vModelCheckbox, vModelCheckboxInit, vModelCheckboxUpdate, vModelDynamic, vModelRadio, vModelSelect, vModelSelectInit, vModelSetSelected, vModelText, vModelTextInit, vModelTextUpdate, vShow, vShowHidden, vShowOriginalDisplay, validateComponentName, validateProps, vaporInteropPlugin, version, warn, warnPropMismatch, watch, watchEffect, watchPostEffect, watchSyncEffect, withAsyncContext, withCtx, withDefaults, withDirectives, withKeys, withMemo, withModifiers, withScopeId, withVaporCtx, withVaporDirectives };
//# sourceMappingURL=vue.runtime.esm-bundler-XjKUArZT.js.map